...
inline Napi::Value to_value(Napi::Env env, const nlohmann::json& value)
{
    switch (value.type()) {
    ...
    case nlohmann::json::value_t::object: {
        ...
        for (auto& pair : value.items())
            object.Set(pair.key(), to_value(env, pair.value()));
...
inline nlohmann::json from_value(const Napi::Value& value)
{
    ...
    case napi_object: {
        if (value.IsArray()) {
            ...
            for (uint32_t i = 0; i < length; ++i)
                result.push_back(from_value(array.Get(i)));
        } else {
            ...
            for (uint32_t i = 0; i < length; ++i) {
                ...
                std::string name = from_value<std::string>(key);

                result[name] = from_value(object.Get(key));
            }
        }
...
public:
    ...
    {
        for (auto& pair : value.items())
            opt_.Set(pair.key(), to_value(opt_.Env(), pair.value()));
    }
